    ...
    <span className="inline-flex flex-wrap items-center gap-1 ml-2 align-middle not-prose">
      {RATINGS.map((r) => (
        <button
          key={r.value}
          type="button"
          onClick={() => void castVote({ slot, rating: r.value, voterKey: key })}
          className={`btn btn-xs h-5 min-h-0 px-1.5 text-[10px] leading-none ${mine === r.value ? "btn-primary" : "btn-ghost opacity-50 hover:opacity-100"}`}
