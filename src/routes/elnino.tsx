  ...
  return (
    <div ref={ref} className="relative"
      ...
      <div id={id} hidden={!open}
        ...
        <Link to="/elnino" search={{ mode: "review" }} hash={`working-${e.key}`} className="text-xs underline">
...
function Byline() {
  return (
    <p className="text-sm -mt-6 mb-6 not-prose">
      ...
      <span className="opacity-70"> first pass, likely has errors. Switch to Review to vote on each piece.</span>
    </p>
  );
}
...
function ModeToggle({ review }: { review: boolean }) {
  ...
  return (
    <div className="join mb-6 not-prose" role="group" aria-label="Page mode">
      <Link to="/elnino" search={{}} className={tab(!review)} aria-current={review ? undefined : "page"}>Read</Link>
      <Link to="/elnino" search={{ mode: "review" }} className={tab(review)} aria-current={review ? "page" : undefined}>Review</Link>
    </div>
  );
}
...
function Statement({ slot, votes, children }: { slot: string; votes: Vote[]; children: ReactNode }) {
  return (
    <li className="flex flex-col gap-1.5 border-t border-base-200 py-2.5 first:border-t-0 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
...
function Review() {
  ...
  return (
    <section className="not-prose max-w-3xl mb-10">
      <h2 className="text-xl font-semibold tracking-tight mb-1">Review the statements</h2>
      <p className="text-sm opacity-70 mb-3">
        The three California numbers rest on the {ALL_SLOTS.length} statements below. Vote on each: for a
        quote, does the source say it; for data, does the working reproduce it; for our number, does the
        ...
        {CALIFORNIA_ESTIMATES.map((e) => (
          <div key={e.key} id={`working-${e.key}`} className="card bg-base-100 scroll-mt-4">
            <div className="card-body p-5">
              ...
              <p className="text-sm grow-0"><span className="font-medium">We would resolve to yes:</span> {e.definition}</p>
              ...
              <StatementGroup title="Our judgment">
                <Statement slot={oursSlot(e)} votes={votes}>
                  <span className="font-medium">Our number: {e.headline}</span>
    ...
    <section className="mt-10 not-prose card bg-base-100">
      <div className="card-body p-5 text-sm">
        ...
        <p className="opacity-70 grow-0">
          ...
          exist. {ALL_SLOTS.length} statements (data, quotes, sources and our own reasoning) sit behind the
          three numbers; {checkedCount(votes)} have been checked by a reader so far.
        </p>
        <Link to="/elnino" search={{ mode: "review" }} className="btn btn-sm btn-neutral w-fit">Review them</Link>
      </div>
    </section>
  );
}
...
function ElNinoPage() {
  ...
  return (
    <TopicDashboard
      ...
      showCharts={!review}
      intro={review
        ? <><Byline /><ModeToggle review /><Review /></>
        : <><Byline /><ModeToggle review={false} /><CaliforniaEstimates /><EnsoContext /></>}
      footer={<>{!review && <ReviewPointer />}<div className="mt-6"><Caveats topic="elnino" /></div><SuggestionsPanel topic="elnino" /></>}
    />
  );
}
