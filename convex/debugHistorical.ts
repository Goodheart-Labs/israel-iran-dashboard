...
export const debugPolymarketHistory = action({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      console.log("2. Event response:", JSON.stringify(events, null, 2).slice(0, 500));
      ...
      for (const fidelity of fidelityTests) {
        ...
        results.theories[`fidelity_${fidelity}_status`] = historyResponse.status;
        ...
        if (historyResponse.ok) {
          ...
          if (dataPoints.length > 0) {
            ...
            for (let i = 1; i < dataPoints.length; i++) {
              ...
              if (timeDiff > expectedDiff * 1.5) { // 50% tolerance
                gaps.push({
          ...
          results.theories[`fidelity_${fidelity}_error`] = errorText.slice(0, 200);
        }
